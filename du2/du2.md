...
_Nesmiete_ funkciám meniť prototyp ani ich zmazať, môžete však doplniť ďalšie funkcie podľa potreby.
Váš súborový systém by mal zmeny okamžite zapisovať na disk. Nesnažíme sa optimalizovať výkon, ale zabezpečiť spoľahlivosť:
ak napr. vaša funkcia `fs_creat` povie, že súbor bol vytvorený, na disku už musia byť uložené (pomocou `hdd_write`) údaje o tomto súbore,
resp. všetky zmeny súborového systému, ktoré s týmto vytvorením nového súboru súvisia.
...
Programy sa odovzdávajú e-mailom na adresu `jan.mazak@fmph.uniba.sk`, predmet `OS-DU2`, súbor `filesystem.c` priložte k e-mailu. Odovzdať je možné viackrát (najviac raz denne).

Najneskorší možný termín odovzdania je **18. 5. 2026**. Nepodceňujte čas potrebný na túto DÚ, ani celý deň nemusí stačiť.

Oficiálny výstup z testovača bude zverejnený po odovzdaní všetkých domácich úloh na http://dcs.fmph.uniba.sk/~mazak/vyucba/os/results2.html.
